...
export class InquiryModel {
  static async createInquiry(payload) {
    ...
    if (hasSupabaseConfig && supabase) {
      const insertWithRating = async (row) => supabase
        .from('contact_inquiries')
        .insert([row])
        .select('*')
        .single();

      let result = await insertWithRating(normalizedPayload);

      if (result.error && /rating/i.test(result.error.message) && /schema cache|column/i.test(result.error.message)) {
        const { rating, ...withoutRating } = normalizedPayload;
        result = await insertWithRating(withoutRating);
      }
    ...
    const id = Date.now();
    const entry = { id, status: 'new', submitted_at: new Date().toISOString(), ...normalizedPayload };
    list.push(entry);
